...
let u_anonTypeTaggedUnionInfo st =
    let (commonAncestor, unsortedIndices) = u_tup2 u_ty (u_array u_int) st
    AnonTypeTaggedUnionInfo.Create(commonAncestor, unsortedIndices)
...
let _ =
    fill_p_ty2 (fun isStructThisArgPos ty st ->
        ...
        | TType_anon_type_tagged_union (unionInfo, l) ->
            p_byte 10 st
...
let _ =
    fill_u_ty (fun st ->
        ...
        | 10 ->
            let anonTypeTaggedUnionInfo = u_anonTypeTaggedUnionInfo st
            let l = u_tys st
            TType_anon_type_tagged_union(anonTypeTaggedUnionInfo, l)
